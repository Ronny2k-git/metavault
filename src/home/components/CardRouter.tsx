...
export function CardRouter({ title, img, desc, path, onSelectTab, onNavigate, tab, className }: CardRouterProps) {
  ...
  return (
    <div onClick={handleClick}>
      <Card variant={'disabled'} className={twMerge('relative max-md:h-38 h-60 rounded-2xl p-4', className)}>
        <div className="flex h-full flex-col justify-between">
          <div className="flex items-center justify-between">
            <h1 className="text-xl break-words font-semibold text-indigo-300">{title}</h1>
          ...
          <div className="hidde md:flex max-md:absolute bottom-2 right-2 md:mt-2 items-center justify-center">
            <img className="h-12 w-12 max-md:h-7 max-md:w-7" src={img} />
          </div>
