...
export function CountDownClock() {
  ...
  const days = Math.max(Math.floor(now / 1000 / 60 / 60 / 24), 0)
  const hours = Math.max(Math.floor(now / 1000 / 60 / 60) % 24, 0)
  const minutes = Math.max(Math.floor(now / 1000 / 60) % 60, 0)
  const seconds = Math.max(Math.floor((now / 1000) % 60), 0)
    ...
    <div className="flex flex-col ml-12">
      ...
      <div className="flex text-live-accent text-base">
        <div className="flex flex-col items-center mr-1">
          ...
          <span className="text-white text-xs">{days <= 1 ? 'day' : 'days'}</span>
        </div>
        <span>/</span>
